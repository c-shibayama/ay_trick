...
def Run(ct,*args):
  ...
  with ct.attr_locker:
    if len(args)==0:
      PrintDict(ct.attributes,col=col)
    else:
      command= args[0]
      args= args[1:]
      if command=='list':
        if len(args)==0:    PrintDict(ct.attributes, col=col)
        elif len(args)==1:  PrintDict(ct.attributes,max_level=args[0], col=col)
        else:  raise Exception('Invalid arguments for %r: %r'%(command,args))
      elif command=='keys':
        if len(args)==0:
          PrintDict(ct.attributes,keyonly=True, col=col)
        else:
