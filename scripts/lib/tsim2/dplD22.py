...
def Help():
  return '''Dynamic Planning/Learning for grasping and pouring in ODE simulation
    ...
    Based on tsim2.dplD21, modified I/O features of dynamical models.
    Some of them use combined function approximators.
  ...
  while True:  #Try-and-error starts from here.
    ...
    if selected_skill=='std_pour':
      ...
      with sim.TPause(ct):  #Pause during plan/learn
        ...
        ##WARNING:NOTE: Famount5 uses 'lp_pour' as input, so here we use a trick:
        #xs_in= CopyXSSA(l.xs.prev)
        #xs_in['lp_pour']= l.xs.n5['lp_pour']
