...
def HelpKeyOp():
  ...
  A: Add a list of arrows (AddArrowList).
  c: Add a cube (AddCube).
  C: Add a list of cubes (AddCubeList).
  s: Add a sphere (AddSphere).
  S: Add a list of spheres (AddSphereList).
    ...
    while not rospy.is_shutdown():
      if kbhit.IsActive():
        ...
        if key=='q':  break
          ...
          scale= np.random.uniform(0,0.5,3)
          mid= viz.AddCubeList(points=points, scale=scale, rgb=RandColL(N), alpha=alpha, mid=mid)
        elif key=='s':
          ...
          points= [RandP() for i in range(N)]
          scale= (lambda r: [r,r,r])(np.random.uniform(0,0.5))
          mid= viz.AddSphereList(points=points, scale=scale, rgb=RandColL(N), alpha=alpha, mid=mid)
        elif key=='y':
          ...
          text= ''.join([chr(np.random.randint(ord('A'),ord('z')+1)) for _ in range(np.random.randint(20))])
          scale= np.random.uniform(0,0.1,1)
          mid= viz.AddText(p=RandP(), text=text, scale=scale, rgb=RandCol(), alpha=alpha, mid=mid)
      else:  break
