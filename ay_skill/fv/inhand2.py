...
def Run(ct,*args):
  ...
  if command=='on':
    ...
    CPrint(1,'Turn on:','vs_inhand2'+LRToStrS(arm))
    ct.thread_manager.Add(name='vs_inhand2'+LRToStrS(arm), target=lambda th_info: ManipLoop(th_info,ct,arm))

  elif command=='off':
    arm= args[0] if len(args)>0 else ct.robot.Arm
    CPrint(2,'Turn off:','vs_inhand2'+LRToStrS(arm))
    ct.thread_manager.Stop(name='vs_inhand2'+LRToStrS(arm))

  elif command=='clear':
    CPrint(2,'Turn off:','vs_inhand2'+LRToStrS(RIGHT))
    CPrint(2,'Turn off:','vs_inhand2'+LRToStrS(LEFT))
